 ...
 *
 * 磁盘布局：
 *   ~/.blexagent/tools/<name>/          工具目录（tool.json + entry，自包含）
 *   ~/.blexagent/tools/registry.json    产品侧状态（enabled/registeredAt）+ description 缓存
 *   ~/.blexagent/bin/<name>             POSIX 启动器（shim，已在所有 runtime 与终端的 PATH 上）
 *   ~/.blexagent/bin/<name>.cmd         Windows 启动器（+ <name>.launcher.cjs）
 *
 * 关键设计：
...
function getBlexAgentDir(): string {
...
export function getCliToolsDir(): string {
  return join(getBlexAgentDir(), 'tools');
}
...
export function getCliToolsBinDir(): string {
  return join(getBlexAgentDir(), 'bin');
}
 ...
 * ~/.blexagent/bin 在 PATH 中先于系统路径（buildClaudeSessionEnv），
...
export function findPathCollision(name: string, effectivePath?: string): string | null {
  ...
  const candidates = process.platform === 'win32'
    ...
    : [name];
  // 显式补上 BlexAgent 自有的 AI 自装 CLI 落点，防止注册工具遮蔽 AI 装过的命令
  const extraDirs = [join(getBlexAgentDir(), 'npm-global', 'bin')];
...
// Auto-generated by \`blexagent tool add\` — do not edit (re-running tool add regenerates it).
// Launcher for ${JSON.stringify(name)}: injects per-tool env from ~/.blexagent/config.json
...
let toolEnv = {};
try {
  const cfg = JSON.parse(readFileSync(join(homedir(), '.blexagent', 'config.json'), 'utf8'));
...
if (result.error) {
  console.error('[blexagent-tool] failed to launch ' + TOOL_NAME + ': ' + result.error.message);
  process.exit(1);
}
...
/**
 * 构建 `<blexagent-user-tools>` 段（纯函数，单测覆盖保险丝行为）。
...
export function buildUserToolsSectionText(tools: ReadonlyArray<{ name: string; description: string }>): string {
  ...
  const overflowLine = overflow > 0
    ? `\n\n…and ${overflow} more registered tool(s) — run \`blexagent tool list\` to see all.`
    : '';
  return `<blexagent-user-tools>
The user has registered the following CLI tools in BlexAgent. They are on PATH —
