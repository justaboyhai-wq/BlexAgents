...
export function getManagedCodexHome(): string {
  return join(homedir(), '.blexagent', 'codex');
}

function getBlexAgentCliExecutablePath(): string {
  return join(homedir(), '.blexagent', 'bin', process.platform === 'win32' ? 'blexagent.cmd' : 'blexagent');
}
...
export function buildManagedCodexRulesContent(): string {
  const cliPath = getBlexAgentCliExecutablePath();
  const patterns: string[][] = [
    ['blexagent'],
    ['blexagent.cmd'],
    [cliPath],
    ['/bin/zsh', '-lc', 'blexagent'],
    ['/bin/zsh', '-lc', cliPath],
    ['/bin/bash', '-lc', 'blexagent'],
    ['/bin/bash', '-lc', cliPath],
    ['zsh', '-lc', 'blexagent'],
    ['bash', '-lc', 'blexagent'],
    ['cmd.exe', '/d', '/s', '/c', 'blexagent'],
    ['powershell', '-Command', 'blexagent'],
    ['pwsh', '-Command', 'blexagent'],
  ];
  return [
    '# Generated by BlexAgent. Do not edit; it is refreshed when managed Codex starts.',
    '# Allows the built-in BlexAgent CLI control-plane tool to run without sandbox approval.',
...
export function ensureManagedCodexRules(): void {
  ...
  const rulesPath = join(rulesDir, 'blexagent.rules');
...
export function getManagedCodexRuntimeRoot(): string {
  return join(homedir(), '.blexagent', 'runtimes', 'codex');
}
...
function buildManagedCodexEnv(
  ...
  ensureDirSync(join(managedHome, 'blexagent'));
  ...
  env.BLEXAGENT_RUNTIME_SOURCE = 'managed-provider';
  return env;
}
