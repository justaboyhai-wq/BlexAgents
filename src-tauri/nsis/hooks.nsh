...
; plus orphan SDK/MCP processes that reference .blexagent in their command line.
...
!macro _BLEXAGENT_KILL_PROCESSES
  DetailPrint "Cleaning up BlexAgent background processes..."
  ...
  ;    (their executable is NOT under $INSTDIR, but their CommandLine references .blexagent)
  nsExec::ExecToLog 'powershell -NoProfile -Command "$ErrorActionPreference=\"SilentlyContinue\"; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like \"*claude-agent-sdk*\" -and $_.CommandLine -like \"*.blexagent*\" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }"'
  nsExec::ExecToLog 'powershell -NoProfile -Command "$ErrorActionPreference=\"SilentlyContinue\"; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like \"*.blexagent\mcp\*\" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }"'
...
!macro NSIS_HOOK_PREINSTALL
  !insertmacro _BLEXAGENT_KILL_PROCESSES
...
!macro NSIS_HOOK_PREUNINSTALL
  ; Kill all BlexAgent processes before uninstall (same file-lock issue as update)
  !insertmacro _BLEXAGENT_KILL_PROCESSES
