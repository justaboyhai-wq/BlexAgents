...
> 本文档记录了在 BlexAgent 中实现人工干预工具权限（Human-in-the-Loop）时的关键发现和最佳实践。
...
BlexAgent 为了 fullAgency 热切换永远传 `allowDangerouslySkipPermissions:true`，这使原生 CLI `isBypassPermissionsModeAvailable=true`。原生解析器逻辑是 `plan && bypassAvailable → "allow"`——只有走到 `"ask"` 分支才会发 `can_use_tool` 消息。后果：plan 模式下 canUseTool 的只读规则根本不跑，弱模型 `rm -rf` / `Edit` 直接执行，`permission_denials` 为空。
...
后台子 Agent 的工具调用走 SDK `PermissionRequest` hook（消息带 `agent_id`，且 `task_started.task_id === agent_id` 可用于关联后台性），无人放行就**自动拒绝**——症状是委派静默失败，不是挂死。per-agent `permissionMode` 也救不了：BlexAgent 默认 `auto→acceptEdits` 下 per-agent mode 被 SDK 跳过。
