...
使用场景：Provider 不通、npm/plugin 拉包失败、MCP 远程服务不可达、外部 Runtime 在终端能用但 BlexAgent 里不能用、localhost 被代理导致 502。

## Ground truth

- BlexAgent 有自己的代理设置，Rust 启动子进程时会注入 proxy env，并保护 localhost/127.0.0.1 不走代理。
- `runtimeSource=system-cli` 的外部 Runtime 有 `envPolicy.proxy`：`blexagent` 使用 BlexAgent 设置的代理，`terminal` 尝试复现用户交互式 shell 的 proxy env。
...
- 用户终端能访问，BlexAgent 不能：优先查 external runtime `effectiveEnv` 或 BlexAgent proxy 设置，而不是直接让用户重装。
- `terminal` policy 下 proxy 为空：用户 shell 没导出 proxy，不是 BlexAgent 漏读。
...
- 可以建议用户在 BlexAgent 设置里配置代理，或把外部 Runtime 的 proxy policy 切到“跟随终端”。
- 不要要求用户安装系统 Node 来修 BlexAgent 自身进程；最终用户应是 bundled Node 零依赖。
- 不要把代理 secret 原样写进报告。
